...
int checkboard(void)
{
	puts("Board: kepter_wc (Ingenic XBurst M200 SoC)\n");
	return 0;
}
...
void check_power_status(int cur_rle_num, int full_rle_num)
{
        ...
        if ((cur_rle_num + 1) >= full_rle_num) { // full
	        gpio_direction_output(GPIO_PA(10), 0); // blue
		gpio_direction_output(GPIO_PC(19), 0); // green
	} else {
	        gpio_direction_output(GPIO_PA(10), 0); // blue
		gpio_direction_output(GPIO_PC(19), 1); // green
	}
}
