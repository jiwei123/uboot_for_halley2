...
static int sm5007_write(unsigned char addr, u8 reg, uint8_t *val) {
    int ret;

    ret = i2c_write(&sm5007_i2c, addr, reg, 1, val, 1);
    if (ret)
        return ret;
...
int probe_sm5007()
{
    int ret = i2c_probe(&sm5007_i2c, SM5007_PMU_ADDR);;
    if (!ret) {
        /*
         * Enable PWRSTM pin function
         */
        u8 value = 0x8 | (0x1 << 1);
        ret = sm5007_write(SM5007_PMU_ADDR, SM5007_CNTL2, &value);
    }

    return ret;
}

static void sm5007_set_voltage(enum sm5007_regulator_id_t id, int uV)
{
    switch (id)
    {
    case SM5007_BUCK1: {
        u8 value = ((uV - 700000) / 12500) & 0x3f;
        int ret = sm5007_write(SM5007_PMU_ADDR, SM5007_BUCK1CNTL2, &value);
        if (ret)
    ...
    case SM5007_BUCK1_SLEEP: {
        /*
         * set at slot3 when enter/exit suspend
         */
        u8 value = 0x2 | (0x3 << 6);
        int ret = sm5007_write(SM5007_PMU_ADDR, SM5007_BUCK1CNTL1, &value);
        if (ret)
            stop(STOP_ERROR_PMU_WRITE_FAILED);

        value = ((uV - 700000) / 12500) & 0x3f;
        value |= 0x3 << 6;
        ret = sm5007_write(SM5007_PMU_ADDR, SM5007_BUCK1CNTL3, &value);
        if (ret)
            stop(STOP_ERROR_PMU_WRITE_FAILED);
...
void pmu_sm5007_set_core_voltage(int mV)
{
    ...
    sm5007_set_voltage(SM5007_BUCK1, mV * 1000);
}
...
void pmu_sm5007_set_core_sleep_voltage(int mV)
{
    ...
    sm5007_set_voltage(SM5007_BUCK1_SLEEP, mV * 1000);
}
