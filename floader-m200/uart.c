...
{
    /* Use uart3 */
    uart = (struct jz_uart *)(UART0_BASE + CONFIG_CONSOLE_UART_NO * 0x1000);
    ...
    writeb((UART_BAUD_DIV_BEST >> 8) & 0xff, &uart->dlhr_ier);
    writeb(UART_BAUD_DIV_BEST & 0xff, &uart->rbr_thr_dllr);
    writeb(UART_BAUD_UMR_BEST, &uart->umr);
    writeb(UART_BAUD_UACR_BEST, &uart->uacr);
