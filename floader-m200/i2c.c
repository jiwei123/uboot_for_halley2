        ...
        unsigned char *buffer, int len)
{
    ...
    debug("i2c_read: chip %x addr %x alen %d buffer %p len %d\n", chip, addr,
            alen, buffer, len);
    /*
 ...
 * Write bytes
 */
int i2c_write(struct i2c *i2c, unsigned char chip, unsigned int addr, int alen,
        unsigned char *buffer, int len)
{
    int shift, failures = 0;

    debug("i2c_write: chip %x addr %x alen %d buffer %p len %d\n", chip, addr,
            alen, buffer, len);
...
{
    gpio_request(i2c->scl, i2c->name);
    gpio_request(i2c->sda, i2c->name);
    gpio_direction_output(i2c->sda, 1);
    gpio_direction_output(i2c->scl, 1);
    send_reset(i2c);

    send_start(i2c);
    int rc = write_byte(i2c, (addr << 1) | 0);
    send_stop(i2c);

    debug("i2c probe:%d, addr:0x%x\n", rc, addr);
