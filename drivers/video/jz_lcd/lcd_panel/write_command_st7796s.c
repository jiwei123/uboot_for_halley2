...
static struct my_common my_common_enter[COUNT_NUM] = {
	...
	{ "display_on", st7796s_display_on},
	{ "display_out", st7796s_display_off},
	{ "pixels_off", st7796s_set_pixel_off},
	{ "pixels_on", st7796s_set_pixel_on},
};

static int write_to_st7796s(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	int i;
	if(argc < 2) {
	...
	if(strcmp("brightness", argv[1]) == 0) { /* set brightness */
			st7796s_set_brightness(dsi, simple_strtol(argv[2],'\0',10));
	}
	else {
...
U_BOOT_CMD( /* now just only can write command ,can't read*/
	auo_command,	3,	1,	write_to_st7796s, /* the max num of command line param argc be set 3 */
	"write command to ST7796S",
