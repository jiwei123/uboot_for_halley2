...
static inline void jzdma_load_firmware()
{
...
static inline void set_gpioa_intc_by_mcu()
{
	unsigned int mask;
...
static inline void jzdma_mcu_reset()
{
	unsigned int dmcs;
...
static inline void jzdma_mcu_init()
{
	unsigned int dmcs;
	dmcs = readl(PDMA_IOBASE + DMCS);
	dmcs &= ~0x1;
...
int jzdma_prep_memcpy(unsigned short channel, dma_addr_t srcaddr, dma_addr_t dstaddr, unsigned int len)
{
	...
	int dataunit = 4;
	int icsr, tmp;
	//serial_printf(" %s start! srcaddr= %x dstaddr= %x len= %d\n",__func__,srcaddr,dstaddr,len);
...
//	tmp &= ~((0x7 << TSZ_MASK_3BIT) + (0xff << DP_SP_RDIL_MASK_8BIT) + (0x3 << DAI_SAI_MASK_2BIT));
	...
	writel(tmp, regaddr);
	//serial_printf("DCA: %x ----> %x\n",regaddr,tmp);

	/*fill channel DTC*/
	regaddr = get_channel_address(channel, DTC_N);
	tmp = (len + dataunit - 1) / dataunit;
	if(tmp == 0){
		serial_printf("ERROR:invalid DTC!!!!!!!!!!!!!!!! len=%d  dataunit=%d\n",len,dataunit);
		while(1);
	}
	writel(tmp, regaddr);
	//serial_printf("DTC: %x ----> %x\n",regaddr,tmp);

	/*fill channel DRT*/
	regaddr = get_channel_address(channel, DRT_N);
	tmp = PDMA_AUTO_REQUEST;
	writel(tmp, regaddr);
	//serial_printf("DRT: %x ----> %x\n",regaddr,tmp);
