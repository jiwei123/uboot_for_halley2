...
void __speed_dug_begin(int mode, PageList *pl)
{
	...
	if (mode == NDD_READ) {
		rd_sum_bytes += bytes;
		rd_btime = get_time_nsecs();
	} else {
		wr_sum_bytes += bytes;
		wr_btime = get_time_nsecs();
	}
}
...
void __speed_dug_end(int mode)
{
	...
	unsigned long long etime = get_time_nsecs();

	if (mode == NDD_READ) {
		rd_sum_time += (etime - rd_btime);
		if (rd_sum_bytes >= DEBUG_TIME_BYTES) {
			times_ms = div_s64_32(rd_sum_time, 1000 * 1000);
		...
		if (wr_sum_bytes >= DEBUG_TIME_BYTES) {
			times_ms = div_s64_32(wr_sum_time, 1000 * 1000);
