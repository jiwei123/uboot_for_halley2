...
int os_clib_init(os_clib *clib)
{
	...
	ndelay = clib->ndelay;
	if (!ndelay)
		RETURN_ERR(ENAND, "function ndelay() not defined!\n");

	div_s64_32 = clib->div_s64_32;
	if (!div_s64_32)
		RETURN_ERR(ENAND, "function div_s64_32() not defined!\n");
	...
	if (!ndd_free)
		RETURN_ERR(ENAND, "function free() not defined!\n");
	...
	get_time_nsecs = clib->get_time_nsecs;
	if (!get_time_nsecs)
		RETURN_ERR(ENAND, "function get_time_nsecs() not defined!\n");
