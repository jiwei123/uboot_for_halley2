...
int nand_io_open(nfi_base *base, chip_info *cinfo)
{
	...
	if (cinfo) {
		...
		nandio->copy_context = cpu_move_init(&nandio->trans, cinfo->buswidth);
	} else {
		nandio->cinfo = NULL;
		nand_io_setup_default(nandio->base);
		nandio->copy_context = cpu_move_init(&nandio->trans, 8);
	}
...
int nand_io_chip_select(int context, int cs)
{
	...
	//printf("cs = %d, csmemory = %p, dataport = %p\n",
