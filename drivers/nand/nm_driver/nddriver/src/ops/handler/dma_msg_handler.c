...
#define DMA_MAILBOX_NAND (*(volatile unsigned int *)(0xb34247f8))
...
static int wait_dma_finish(unsigned int *mailbox)
{
	...
	do{
		...
		/*
		   ...
		   dump_taskmsg((struct task_msg *)PDMA_MSG_TCSMVA, 4);
		 */
#ifdef MCU_TEST_INTER_NAND
		if(timeout < 0){
...
static int mcu_init(nand_dma *nd_dma,Nand_Task *nandtask,int id)
{
	...
	//ndd_printf("................. size = %d line = %d \n",sizeof(struct taskmsg_init),__LINE__);
...
//	msg->info.eccpos = ndinfo->eccpos;
	...
	//dump_initmsg(msg);
	dma_cache_wback((unsigned int)nd_dma->msg_init, sizeof(struct taskmsg_init));

	//ndd_debug("########  msg_phyaddr = 0x%x # PDMA_MSG_TCSMPA = 0x%x ########\n",msg->info.taskmsgaddr,PDMA_MSG_TCSMPA);
	return send_msg_to_mcu(nd_dma,nd_dma->msginit_phyaddr,sizeof(struct taskmsg_init) * 4);
}
...
static int dma_handle_msg(int context, Nand_Task *nt)
{
	...
	dma_cache_wback((unsigned int)nt->msg, sizeof(struct task_msg) * nt->msg_index);
...
static nand_dma *dma_ops_init(nand_data *data, Nand_Task *nandtask,int id)
{
	...
	return g_nddma;
err:
	ndd_print(NDD_ERROR, "ERROR: Nand DMA ops init FAILED!\n");
	return NULL;
}
...
static void test_dma()
{
	...
	dma_cache_wback((unsigned int)src_data,sizeof(src_data));
	dma_cache_wback((unsigned int)dst_data,sizeof(dst_data));
