...
int cpu_msg_handle_init(nand_data *data, Nand_Task *nandtask, int id)
{
	...
	if (!handler)
		GOTO_ERR(ndd_alloc_handler);

	cpu_msg = ndd_alloc(sizeof(struct cpu_msg_ops));
	if (!cpu_msg)
		GOTO_ERR(ndd_alloc_cpu_msg);
...
ERR_LABLE(ndd_alloc_ffbuf):
	ndd_free(cpu_msg->aa_buf);
ERR_LABLE(ndd_alloc_aabuf):
	ndd_free(cpu_msg->data_buf);
ERR_LABLE(ndd_alloc_databuf):
	ndd_free(cpu_msg->bch_buf);
ERR_LABLE(ndd_alloc_bchbuf):
	ndd_free(cpu_msg);
ERR_LABLE(ndd_alloc_cpu_msg):
	ndd_free(handler);
ERR_LABLE(ndd_alloc_handler):
