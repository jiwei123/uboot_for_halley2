...
static void set_retry_feature(int cycle){}
static void nandops_retry(struct task_msg *msg, int num,int cycle)
{
    union taskmsghead *ops;
    union taskmsgdata *msgdata;
    unsigned int i,cs = 0xffffffff;
    for(i = 0; i < num; i++){
	    ops = &((msg + i)->ops);
	    msgdata = &((msg + i)->msgdata);
	    if(cs != ops->bits.chipsel){
		    cs = ops->bits.chipsel;
		    set_retry_feature(cycle);
	    }
    }
}
...
int nandops_read(int context, ndpartition *pt, PageList *pl)
{
	...
	tmp_pl = pl;
	do{
		ndd_print(NDD_INFO,"%s start, pt[%s] planes[%d] startblockid[%d] eccbit[%d] ops_mode[%d]\n",
			  __func__,pt->name,pt->planes,pt->startblockid,pt->eccbit,pt->ops_mode);
		...
		for(i = 0; i < unit_cnt; i++){
			if(nandtask[i]->msg_index != 0){
				...
				if(retrycnt != 0)
					nandops_retry(nandtask[i]->msg, nandtask[i]->msg_index, retrycnt);
					...
					if(pl_node->retVal == ECC_ERROR){
						if(new_pl == NULL){
							ndd_memset(ops->retrytop, 0x0,
									sizeof(PageList) * ((VNANDCACHESIZE + 512) / 512));
...
//						probe_node->head.next = NULL;
					...
					if(pl_node->retVal == ECC_ERROR){
						if(new_pl == NULL){
							...
							singlelist_add(&(probe_node->head),&(pl_node->head));
							probe_node = pl_node;
						}
						probe_node->head.next = NULL;
					}
	...
	if(retrycnt > 1){
		ndd_print(NDD_DEBUG, "^^^^^ retrycnt = %d ^^^^^^\n",retrycnt);
		index = 0;
		singlelist_for_each(pos, &(tmp_pl->head)){
			pl_node = singlelist_entry(pos, PageList, head);
...
//	ndd_print(NDD_DEBUG,"%s %d blockid[%d],blkcnt[%d]!\n",__func__,__LINE__,bl->startBlock,bl->BlockCount);
...
int nandops_init(nand_data *data)
{
	...
	libcinfo.totalblocks = cinfo->totalblocks;
	libcinfo.totalpages = cinfo->totalpages;
	...
	/*** init retry pagelist ***/
	ops->retrytop = (PageList *)ndd_alloc(sizeof(PageList) * ((VNANDCACHESIZE + 512) / 512));
	if(!ops->retrytop)
...
ERR_LABLE(ndd_alloc_unit):
	ndd_free(ops->retrytop);
ERR_LABLE(retrytop_init):
	lib_nandops_deinit(ops->lib_ops);
ERR_LABLE(lib_ops_init):
	ndd_free(libioinfo);
ERR_LABLE(ndd_alloc_libioinfo):
#if 1
	ndd_free(ops->mem);
ERR_LABLE(ndd_alloc_mem):
#endif
	ndd_free(ops->nandtask);
ERR_LABLE(ndd_alloc_nandtask):
	ndd_free(ops);
ERR_LABLE(ndd_alloc_info):
ERR_LABLE(data):
