...
static void ddr_cfg_init(void)
{
	...
	ddrc_cfg =
		DDRC_CFG_TYPE_DDR3 | DDRC_CFG_IMBA | DDR_DW32 | DDRC_CFG_ODT_EN | DDRC_CFG_MPRT |
		(tmp | 0x8) << 2 |
...
static void ddr_phy_init(unsigned long ps, unsigned int dtpr0_reg)
{
...
#else
	writel(DDR3_MR1_DIC_7 | DDR3_MR1_RTT_4, DDRP_MR1);
#endif
	...
	writel(DDRP_PIR_INIT | DDRP_PIR_DRAMINT | DDRP_PIR_DRAMRST | DDRP_PIR_DLLSRST, DDRP_PIR);
